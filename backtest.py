#read data/wsj_predictions/backtest.json file
import json
import re
import random
import difflib
...
filename = 'data/wsj_predictions/2022-02-11.json'
...
def apply_stock_move(info, s):
    ...
    for word in words:
        ...
        for c_info in info.values():
            if difflib.SequenceMatcher(None, c_info['ticker'], word).ratio() > 0.5 or difflib.SequenceMatcher(None, c_info['company'].lower(), word.lower()).ratio() > 0.46:
                mode = c_info['ticker']
...
def apply_prices_alpaca(info, date):
    ...
    tomorrow = date_obj + datetime.timedelta(hours=1)
    for ticker in info.keys():
...
def process_article(article):
    tickers = get_tickers(article['Stocks Affected'])


    companies = get_company_names(tickers, article['Stocks Affected'])
